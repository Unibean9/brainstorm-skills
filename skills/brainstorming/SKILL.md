...
- Bare paths (e.g. `references/headless.md`) resolve from `{skill-root}`; `{project-root}`-prefixed paths resolve from the project working directory.
- If the host project provides customization, use it. Otherwise use `{project-root}/artifacts/brainstorm` for output and `{skill-root}/assets/brain-methods.csv` for the technique catalog.
- This skill is standalone. Optional project helper scripts may improve persistence, but their absence must never block a session.
