---
name: prd
description: Create, update, or validate a fact-grounded PRD from brainstorm decisions, research, or an existing product brief.
---
# PRD
...
- If no host customization exists, use `{project-root}/artifacts/prd/{topic_slug}/` as `{doc_workspace}` and `{project-root}` as the project root.
- This skill is standalone. Use host helper scripts when present; otherwise perform the equivalent file operations directly and never block on missing BMad tooling.

## Product-pipeline quality bar

When this skill follows a completed brainstorm, produce a complete PRD rather than a short concept note. Unless the product genuinely does not need a section, cover: executive summary, problem, target users, jobs-to-be-done, glossary, user journeys, solution thesis, feature groups, numbered functional requirements, non-functional requirements, scope and sequencing, success and counter-metrics, risks, assumptions, open questions, and acceptance criteria. Preserve the brainstorm's provenance and mark every inference as `[ASSUMPTION]`.
...
**Forwarded activation:** if a caller supplies a stated intent or pre-resolved fields, honor them verbatim — skip duplicate intent inference and resolve only the remaining fields.

1. Resolve customization when the host project provides a resolver; otherwise use the defaults in `## Conventions`.
...
3. Resolve host project configuration when available; otherwise derive the project name from the workspace and use today's date.
4. If headless, follow `references/headless.md` for the whole run. Otherwise greet the user and detect whether the request is Create, Update, or Validate. If it clearly belongs to another installed skill, route it without blocking.
...
**Create.** Bind `{doc_workspace}` to the configured PRD output path or the standalone default. Write `prd.md` with YAML frontmatter (title, status, created, updated — initial `status: draft`), and seed `.memlog.md` using the host helper when available or direct append-only Markdown otherwise. Tell the user the path. Run `## Discovery`, then `## Finalize`.

**Update.** Reconcile the PRD with a change signal. Source-extract against PRD, addendum, `.memlog.md`, and original inputs (extract, don't ingest). If `.memlog.md` is missing, create it and record recovered decisions before continuing. Surface conflicts with prior decisions before applying. Then `## Finalize`.
...
7. **Close.** Set `prd.md` frontmatter `status: final` and `updated` to `{date}` so future invocations distinguish this PRD from in-progress drafts. Record finalization in `.memlog.md`. Share artifact paths and suggest the next installed downstream skill when relevant.
